...
export const CoordinatedGroup = ({
  longitudeLatitudeHeight,
  children,
  ...groupProps
}: CoordinatedGroupProps) => {
  const sceneViewModel = useSceneViewModel();
  const sceneModel = sceneViewModel.sceneModel;
  const groupReference = useRef<Group>(null);
  const [groupLongitudeLatitudeHeight, setGroupLongitudeLatitudeHeight] = useState<Vector3 | null>(
    null
  );
  ...
  useFrame(({}) => {
    ...
    if (
      !!groupLongitudeLatitudeHeight &&
      groupLongitudeLatitudeHeight.equals(longitudeLatitudeHeightVector)
    )
      return;
  ...
  const syncGroup = () => {
    ...
    sceneModel.getScenePositionForLongitudeLatitudeHeight(
      groupLongitudeLatitudeHeight,
      group.position
    );

    // set the group's up to the surface normal
    sceneModel.getSceneSurfaceNormalForLongitudeLatitudeHeight(
      groupLongitudeLatitudeHeight,
      group.up
    );
