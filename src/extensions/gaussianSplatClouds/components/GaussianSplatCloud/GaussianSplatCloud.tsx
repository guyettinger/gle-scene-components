...
import { Viewer as GaussianSplatViewer, SceneRevealMode } from 'gle-gaussian-splat-3d';
...
export const GaussianSplatCloud = ({
  ...
  const gaussianSplatCloudsSceneViewExtension =
    sceneViewModel.getSceneViewExtension<GaussianSplatCloudsSceneViewForegroundLayerExtension>(
      SceneExtensionNames.GaussianSplatClouds
    );
  ...
  useFrame(({ gl, scene, camera, pointer, size }) => {
    ...
    if (!gaussianSplatViewer) {
      ...
      if (
        !gaussianSplatViewer.isLoadingOrUnloading() &&
        !gaussianSplatCloudLoaded &&
        !gaussianSplatCloudLoading
      ) {
        // load gaussian splat cloud
        setGaussianSplatCloudLoading(true);

        gaussianSplatViewer
          .addSplatScene(baseUrl + fileName, {
            ...
            if (splatMesh) {
              ...
              if (group) {
                ...
                group.raycast = (raycaster, intersects) => {
                  ...
                  // perform raycast
                  const outHits: any[] | undefined = gaussianSplatViewer?.performRaycast(
                    camera,
                    mousePosition
                  );
                  outHits?.forEach((hit) => {
  ...
  return (
    <group
      {...groupProps}
      ref={gaussianSplatCloudGroupReference}
