import { Cartesian3, Matrix4 as CesiumMatrix4, Ellipsoid, Transforms } from 'cesium';
import { Vector3 } from 'three';

export const offsetCartesianPositionByCartesianOffset = (
  center: Cartesian3,
  offset: Cartesian3,
  destination: Cartesian3 = new Cartesian3()
): Cartesian3 => {
  const transform = Transforms.eastNorthUpToFixedFrame(center);
  return CesiumMatrix4.multiplyByPoint(transform, offset, destination);
};

export const offsetCartesianPositionBySceneOffset = (
  center: Cartesian3,
  offset: Vector3,
  destination: Cartesian3 = new Cartesian3()
): Cartesian3 => {
  return offsetCartesianPositionByCartesianOffset(
    center,
    new Cartesian3(offset.x, -offset.z, offset.y),
    destination
  );
};

export const getOffsetEastNorthUpCartesian = (
  centerCartesian: Cartesian3,
  destinationCartesian: Cartesian3,
  offsetEastNorthUpCartesian: Cartesian3 = new Cartesian3()
) => {
  ...
  offsetEastNorthUpCartesian = CesiumMatrix4.multiplyByPointAsVector(
    CesiumMatrix4.inverse(transform, new CesiumMatrix4()),
    offset,
    offsetEastNorthUpCartesian
  );

  return offsetEastNorthUpCartesian;
};

export const getScenePositionForLongitudeLatitudeHeight = (
  sceneCenter: Vector3,
  sceneCenterCartesian: Cartesian3,
  longitudeLatitudeHeight: Vector3,
  scenePosition = new Vector3()
): Vector3 => {
  const scenePositionCartesian = Cartesian3.fromDegrees(
    longitudeLatitudeHeight.x,
    longitudeLatitudeHeight.y,
    longitudeLatitudeHeight.z
  );
  return getScenePositionForCartesian(
    sceneCenter,
    sceneCenterCartesian,
    scenePositionCartesian,
    scenePosition
  );
};

export const getScenePositionForCartesian = (
  sceneCenter: Vector3,
  sceneCenterCartesian: Cartesian3,
  scenePositionCartesian: Cartesian3,
  scenePosition = new Vector3()
): Vector3 => {
  const offsetEastNorthUp = getOffsetEastNorthUpCartesian(
    sceneCenterCartesian,
    scenePositionCartesian
  );
  scenePosition.set(
...
export const getSceneSurfaceNormalForLongitudeLatitudeHeight = (
  longitudeLatitudeHeight: Vector3,
  sceneSurfaceNormal = new Vector3()
): Vector3 => {
  const scenePositionCartesian = Cartesian3.fromDegrees(
    longitudeLatitudeHeight.x,
    longitudeLatitudeHeight.y,
...
export const getSceneSurfaceNormalForCartesian = (
  scenePositionCartesian: Cartesian3,
  sceneSurfaceNormal = new Vector3()
): Vector3 => {
